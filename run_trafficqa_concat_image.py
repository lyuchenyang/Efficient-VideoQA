""" running training and evaluation code for audio-aware dialogue-to-video retrieval
...
from modeling import AADVConcat
import clip
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            for vid in train_video_ind:
                frame = preprocess(
                    Image.open('{}{}.jpg'.format(args.image_dir, train_video_names[str(vid)].replace('.mp4', ''))))
...
def evaluate(args, model, preprocess, eval_dataset, prefix=""):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            for vid in eval_video_ind:
                frame = preprocess(Image.open('{}{}.jpg'.format(args.image_dir, eval_video_names[str(vid)].replace('.mp4', ''))))
                # frame = preprocess(
    ...
    print(all_ans.shape, all_sim_matrix.shape)

    top_1 = top_k_accuracy_score(all_ans, all_sim_matrix, k=1)

    print('Accuracy: {}'.format(str(top_1*100)))
    print()
    d = {'preds': all_sim_matrix.tolist(), 'labels': all_ans.tolist()}
    json_dump(d, 'eval_results/0104_full_trafficqa.json')
    return
...
def main():
    ...
    clip_processor_name = "ViT-L/14@336px"  # "ViT-L/14@336px"
    clip_model_name = 'openai/clip-vit-large-patch14-336'
    clip_config = CLIPConfig.from_pretrained(clip_model_name)
    args.transformer_width = clip_config.projection_dim

    args.image_dir = 'data/frames_resize_16/'
    args.eval_image_dir = 'data/frames_resize/'
    ...
    model = AADVConcat(args, clip_config)
